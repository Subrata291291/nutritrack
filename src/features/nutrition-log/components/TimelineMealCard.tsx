  ...
  if (empty) {
    return (
      <>
        ...
        <Modal open={pendingDeleteId !== null} onClose={() => setPendingDeleteId(null)} title="Remove Food">
          <p className="text-sm text-on-surface-variant mb-6">Are you sure you want to remove this food from your log?</p>
              ...
              {items.map((entry) => (
                <div key={entry.id} className="flex items-center justify-between px-xl py-3 hover:bg-surface-container-low/30 transition-colors group/item">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <button onClick={() => setPendingDeleteId(entry.id)} className="opacity-0 group-hover/item:opacity-100 p-1.5 text-on-surface-variant hover:text-error rounded-lg hover:bg-error-container/20 transition-all" title="Remove">
